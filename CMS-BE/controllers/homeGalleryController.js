...
// Create new entry
exports.createHomeGallery = async (req, res) => {
    try {
        ...
        const imageUrl = `${baseUrl}/uploads/home-gallery/${req.file.filename}`;

        const entry = new HomeGallery({
            imageTitle,
            description,
            category,
            imageUrl,
        ...
        res.status(201).json({
            success: true,
            message: 'Home Gallery item uploaded successfully',
            data: normalizeEntry(entry, req)
        });
    } catch (error) {
        console.error('Error creating home gallery item:', error);
        res.status(500).json({
            success: false,
            message: 'Failed to upload home gallery item',
            error: error.message
        });
    }
};
...
// Get all entries
exports.getAllHomeGallery = async (req, res) => {
    try {
        ...
        const entries = await HomeGallery.find(filter)
        ...
        res.status(500).json({
            success: false,
            message: 'Failed to fetch home gallery items',
            error: error.message
        });
    }
};
...
// Get single entry
exports.getHomeGalleryById = async (req, res) => {
    try {
        const entry = await HomeGallery.findById(req.params.id);
...
exports.updateHomeGallery = async (req, res) => {
    try {
        ...
        if (req.file) {
            const baseUrl = process.env.BASE_URL || getBaseUrl(req);
            entry.imageUrl = `${baseUrl}/uploads/home-gallery/${req.file.filename}`;
        }
