...
const DonationGalleryForm = () => {
    ...
    const fetchDetails = async () => {
        try {
            ...
            const response = await fetch(`https://api.harekrishnavidya.org/api/donation-gallery/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const url = isEditMode ? `https://api.harekrishnavidya.org/api/donation-gallery/${id}` : 'https://api.harekrishnavidya.org/api/donation-gallery';
            const res = await fetch(url, { method: isEditMode ? 'PUT' : 'POST', body: submitData });
            if (!res.ok) throw new Error('Save failed');
            navigate('/donation-gallery');
        } catch (err) {
        ...
        <div className="min-h-screen bg-gray-50">
            <div className="bg-white border-b px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate('/donation-gallery')} className="text-gray-600"><FaArrowLeft /></button>
                    <h1 className="text-xl font-bold">{isEditMode ? 'Edit Donation Item' : 'Add Donation Item'}</h1>
                </div>
...
export default DonationGalleryForm;
