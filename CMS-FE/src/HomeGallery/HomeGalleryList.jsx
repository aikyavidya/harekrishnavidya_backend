...
const HomeGalleryList = () => {
    ...
    const fetchPhotos = async () => {
        try {
            setLoading(true);
            const response = await fetch(`https://api.harekrishnavidya.org/api/home-gallery`);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const response = await fetch(`https://api.harekrishnavidya.org/api/home-gallery/${id}`, { method: 'DELETE' });
    ...
    return (
        <div className="min-h-screen bg-gray-50 p-6">
            <div className="flex items-center justify-between mb-8">
                ...
                <button onClick={() => navigate('/home-gallery/create')} className="px-6 py-2 bg-blue-600 text-white rounded-lg flex items-center gap-2">
                    <FaPlus /> Add New
                </button>
            </div>
                    ...
                    {photos.map((photo) => (
                        <div key={photo._id} className="bg-white rounded-xl border overflow-hidden flex flex-col shadow-sm">
                            ...
                            <div className="p-3 bg-gray-50 border-t grid grid-cols-2 gap-2">
                                <button onClick={() => navigate(`/home-gallery/edit/${photo._id}`)} className="flex items-center justify-center gap-2 py-2 bg-white border rounded text-sm font-semibold text-blue-600 hover:bg-blue-50">
                                    <FaEdit /> Edit
                                </button>
